<template>
  <div
    ...
    <slot v-bind:rect="{ left, top, width, height }"></slot>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleDrag(e) {
      ...
      const dr = this.onDrag(left, top);
      if (typeof dr == "object") {
        if (dr.left == left && dr.top == top) {
          return;
        }
        left = dr.left;
        top = dr.top;
    ...
    moveHorizontally(val) {
      ...
      const delta = snapToGrid(this.grid, val, this.top, 1);
      const left = restrictToBounds(delta[0], this.bounds.minLeft, this.bounds.maxLeft);
    ...
    moveVertically(val) {
      // should calculate with scale 1.
      const delta = snapToGrid(this.grid, this.left, val, 1);
      const top = restrictToBounds(delta[1], this.bounds.minTop, this.bounds.maxTop);
    ...
    changeWidth(val) {
      // should calculate with scale 1.
      const delta = snapToGrid(this.grid, val, 0, 1);

      let right = restrictToBounds(this.parentWidth - delta[0] - this.left, this.bounds.minRight, this.bounds.maxRight);
      let bottom = this.bottom;
    ...
    changeHeight(val) {
      // should calculate with scale 1.
      const delta = snapToGrid(this.grid, 0, val, 1);

      let bottom = restrictToBounds(
        this.parentHeight - delta[1] - this.top,
