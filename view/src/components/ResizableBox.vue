...
export default {
  ...
  computed: {
    style() {
      let s = {
        transform: `translate(${this.l}px, ${this.t}px)`,
        width: `${this.w}px`,
        height: `${this.h}px`,
  ...
  methods: {
    ...
    // fixAspectRatio(before, after) {
    //   const oldAspect = this.width / this.height;
    //   const newAspect = after.w / after.h;
    //   if (newAspect == oldAspect) {
    //     return after;
    //   }
    //   if (after.w * after.h > before.w * before.h) {
    //     if (newAspect > oldAspect) {
    //       const newH = w / oldAspect;
    //       if (before.h != after.h) {

    //       } else {
    //         // update t
    //       }
    //       return { w, h: w / oldAspect };
    //     } else {
    //       return { w: h * oldAspect, h };
    //     }
    //   } else {
    //     if (newAspect > oldAspect) {
    //       return { w: h * oldAspect, h };
    ...
    handleMove(event) {
      ...
      // const oldAspect = this.width / this.height;
      // if (diffX != 0) {
      //   moveDiffY = diffX / oldAspect;
      // } else if (moveDiffY != 0) {
      //   moveDiffX = oldAspect * moveDiffY;
      // }

      this.offsetX = this.offsetY = 0;
      // const before = { t: this.t, l: this.l, w: this.w, h: this.h };
      ...
      // console.log(`${this.offsetX} ${this.offsetY} diff = ${diffX} ${diffY}`);
      this.mouseX = eventX;
      this.mouseY = eventY;
      // const after = { t: this.t, l: this.l, w: this.w, h: this.h };
      // const fixed = this.fixAspectRatio(before, after);
    ...
    handleDown(event) {
      ...
      for (let elClass in ELEMENT_MASK) {
        if (
          ...
          ((event.target.closest && event.target.closest(`.${elClass}`)) || event.target.classList.contains(elClass))
        ) {
